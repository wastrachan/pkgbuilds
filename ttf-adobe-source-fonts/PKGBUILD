...
_source_url="https://github.com/adobe-fonts"
source=("${pkgname}-${pkgver}-source-code-pro.tar.gz::${_source_url}/source-code-pro/archive/refs/tags/${_pkgver_source_code_pro}.tar.gz"
        "${pkgname}-${pkgver}-source-serif.tar.gz::${_source_url}/source-serif/archive/refs/tags/${_pkgver_source_serif}.tar.gz"
        "${pkgname}-${pkgver}-source-sans.tar.gz::${_source_url}/source-sans/archive/refs/tags/${_pkgver_source_sans}.tar.gz")
noextract=("${source[@]%%::*}")
...
package() {
    ...
    # Install fonts from all directories
    for path in *; do
        if [ -d "$path" ]; then
            # Install regular TTF fonts
